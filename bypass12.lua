...
local nInspectorBroadcastCDSeconds = 30
local nInspectorBroadcastLimitTipsID = 666025
local nHawkEyeInspectType = {BR = 0, Metro = 1}
...
function ClientHawkEyePatrolSubsystem:_PostConstruct()
  ...
  self:AddCommonEvent(EVENTTYPE_SECURITY, EVENTID_SECURITY_INSPECTOR_BROADCAST_RESULT, self._OnInspectorBroadcastResult, self)
...
function ClientHawkEyePatrolSubsystem:_OnRecvInspectorBroadcastCount(_, __, nBroadcastCount, bSendHawkReportBoardcast, nBroadcastLimit)
  self.nInspectorBroadcastCount = nBroadcastCount or -1
  if type(nBroadcastLimit) == "number" and 0 < nBroadcastLimit then
    self.nMaxInspectorBroadcastCount = nBroadcastLimit
  end
  FormatLog("nBroadcastCount[%s] self.nInspectorBroadcastCount[%s] bSendHawkReportBoardcast[%s] nBroadcastLimit[%s] self.nMaxInspectorBroadcastCount[%s]", nBroadcastCount, self.nInspectorBroadcastCount, bSendHawkReportBoardcast, nBroadcastLimit, self.nMaxInspectorBroadcastCount)
  if bSendHawkReportBoardcast then
    ...
    if WarzoneHandle and WarzoneHandle.send_hawkeye_report_broadcast then
      FormatLog("send_hawkeye_report_broadcast broadcast_count[%s]", self.nInspectorBroadcastCount)
      WarzoneHandle.send_hawkeye_report_broadcast(self.nInspectorBroadcastCount)
    end
  end
end

function ClientHawkEyePatrolSubsystem:_OnInspectorBroadcastResult(_, __, nResult)
  FormatLog("_OnInspectorBroadcastResult nResult[%s]", tostring(nResult))
  if nResult == EInspectorBroadcastResult.Broadcasted then
    self.nLastInspectorBroadcastTime = os.time()
    FormatLog("inspector broadcast fired, record nLastInspectorBroadcastTime[%s]", self.nLastInspectorBroadcastTime)
  elseif nResult == EInspectorBroadcastResult.ReachLimit then
    FormatLog("inspector broadcast reach limit, show tips[%s]", nInspectorBroadcastLimitTipsID)
    IngameTipsTools.BattleNormalTipsByTextID(nInspectorBroadcastLimitTipsID)
  else
    FormatLog("unknown inspector broadcast result nResult[%s]", tostring(nResult))
  end
end

function ClientHawkEyePatrolSubsystem:GetInspectorBroadcastCDRemainSec()
  if not self.nLastInspectorBroadcastTime or self.nLastInspectorBroadcastTime <= 0 then
    return 0
  end
  local nElapsed = os.time() - self.nLastInspectorBroadcastTime
  local nRemain = nInspectorBroadcastCDSeconds - nElapsed
  if nRemain <= 0 then
    return 0
  end
  local nCeilRemain = math.ceil(nRemain)
  if nCeilRemain < 1 then
    nCeilRemain = 1
  end
  return nCeilRemain
end
...
function ClientHawkEyePatrolSubsystem:_GetInspectType()
  local PlayerPrefsSystem = require("client.logic.LogicPlayerPrefs.playerprefs")
  local saved = PlayerPrefsSystem.LoadFileToTable_N(PlayerPrefsSystem.ePlayerPrefsType.eEagle)
  if saved then
    local uid = tostring(DataMgr.roleData.uid)
    if saved[uid] and saved[uid].inspectType ~= nil then
      return saved[uid].inspectType
    end
  end
  return self._nBattleInspectType
end

function ClientHawkEyePatrolSubsystem:_IsMetroBattle()
  local GameMainConfig = require("GameLua.GameCore.Main.GameMainConfig")
  local sModType = GameMainConfig.GetModType()
  return type(sModType) == "string" and string.find(sModType, "TPlan", 1, true) ~= nil
end
...
function ClientHawkEyePatrolSubsystem:_InitHawkEyePatrolSubsystem()
  ...
  self._nBattleInspectType = self:_IsMetroBattle() and nHawkEyeInspectType.Metro or nHawkEyeInspectType.BR
  FormatLog("_nBattleInspectType=%s", self._nBattleInspectType)
...
function ClientHawkEyePatrolSubsystem:ExitWatching()
  ...
  if WarzoneHandle and WarzoneHandle.send_leave_hawkeye_watch then
    WarzoneHandle.send_leave_hawkeye_watch("player_exit", self:_GetInspectType())
  end
...
function ClientHawkEyePatrolSubsystem:SendReportTLog(tReasonCodeArray, bInspectorBroadcast, tMainReason)
  ...
  if IsTable(tMainReason) then
    tMainReasonArray = tMainReasonArray or {}
    for _, nReasonCode in ipairs(tMainReason) do
      table.insert(tMainReasonArray, nReasonCode)
    end
  end
...
function ClientHawkEyePatrolSubsystem:WantMatchNextPatrol()
  ...
  local WarzoneHandle = require("client.network.Protocol.WarzoneHandle")
  WarzoneHandle.send_leave_hawkeye_watch("is_hawkeye_next", self:_GetInspectType())
...
function ClientHawkEyePatrolSubsystem:_OnPlayerKilledOtherPlayer(FatalDamageParameter)
  ...
  if slua.isValid(uCurPlayerState) and uCurPlayerState.HasAnyReviveChance and uCurPlayerState:HasAnyReviveChance() == false then
